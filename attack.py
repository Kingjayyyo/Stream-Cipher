...
def PRG_attack(IV, x):
    ...
    # The length to do the shifts for IV
    l = len(IV)
    
    # Go through the different possible shifts    
    for j in range(l):
        # Initialize a list to store the value of j in a list 
        listJ = [j]
        # Assign the state of each init to an array
        initState = init(IV, listJ)
        # List that stores the final result of the next alg
        nxtList = list()
        # iteration for the next state
        count = 0
        # Creating the various result using the next alg.
        for k in range(len(x)):
            tmp = next(count, initState[1])
            nxtList.append(tmp[0]) 
            count = tmp[1]
        if(nxtList == x):
            return 0
...
def EAV_attack(m0, m1, c):
    ...
    #Split the ciphertext into two
    IVlist = c[0:256]
    appendList = c[256:]
    
    #Initialize a count
    count = 0
    # Running a loop that is used for comparison
    for i in IVlist:
        count2 = i
        IViter = count
        appendIter = 0
        for k in appendList:
            if (count2 == k):
                appendIter += 1
                IViter = (IViter + IVlist[(IViter + 1) % 256]) % 256
                count2 = IVlist[IViter]
        if appendIter == len(m0):
            return 0
        count += 1
...
def decrypt(m_list,c):
    ...
    # Creating lists to store different values used to compare
    compKey = list()
    compM = list()
    # Initialize a count
    count = 0
    ...
    for j in m_list:
        # Initialize a key used for comparison
        num = 0
        # Go through the IV extracted from c
        for k in IVlist:
            if (appendList[0] == (k + j[0]) % 256):
                compKey = [num]
                compM = dec(compKey, c)
                if(compM == j):
                    return count
